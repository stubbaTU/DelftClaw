...
|---|---|---|---|
| `host.env` | **Per-host overrides** — Tailscale IPs, `QWEN_BASE_URL`/`QWEN_MODEL`, `BTC_NETWORK`. Machine-specific values that differ across dev VPSes. | `deploy/scenario_boot.py`, `deploy/vps/bootstrap_security_identity.sh`. Loaded if present; ignored otherwise. | **NO** (gitignored). Copy from `host.env.example` and edit. |
