...
BIP-39 seed derives, via a BIP-32 chain, the transport, application,
and wallet keys:
...
- `Seed` + four loaders: `MnemonicSeedSource` (BIP-39),
  ...
  production default).
- `Wallet.from_seed(seed, network, agent_index)` - deterministic
  synthetic wallet wrapper. CLI: `python -m identity.wallet --mnemonic
  '...' {address,balance,send}`. Also runs at scenario boot to
  materialise per-agent seed files.
