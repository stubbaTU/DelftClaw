...
from signed_log.integration.peer_transport import HttpPeerTransport, PeerTransport
...
def build_app(
    ...
    * ``pull_interval`` / ``pull_batch`` — driver knobs forwarded to
      :func:`signed_log.integration.pull_loop.run_pull_loop`.
...
def main() -> None:
    """Module CLI entry point. Used by ``python -m signed_log.integration.server``.
    ...
    parser = argparse.ArgumentParser(prog="signed_log.integration.server")
    parser.add_argument(
