...
class SignedAppendOnlyLog(_LegacyAppendOnlyBase):
    ...
    * ``signature`` — hex-encoded Ed25519 signature over the canonical JSON
    ...
    def __init__(
        ...
        # populated.
        #
        # CROSS-PROCESS NOTE: the signed_log FastAPI server
        # (agent/cli.py:_start_signed_log_server) builds a SEPARATE
    ...
    def verify_integrity(self) -> tuple[bool, list[str]]:
        ...
        ``signed_log.primitives.verify._verify_entry`` (different file, no
    ...
    # ------------------------------------------------------------------
    # Witness-entry helpers (mirrored by name in signed_log.primitives.verify)
    # ------------------------------------------------------------------
