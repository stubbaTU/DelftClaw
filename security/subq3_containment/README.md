...
If the VPS is not prepared yet, install the real isolation dependencies first:

```bash
bash deploy/setup_real_isolation.sh
```
...
```bash
python -m security.subq3_containment.generate_attack_suite \
...
```bash
python -m security.subq3_containment.run_containment_tests \
  --generate-attacks \
