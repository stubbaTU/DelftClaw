...
# --- VENDOR IMPORT SWAPS (redteam-ablation) ---------------------------------
# Vendored from DelftClaw redteam/primitives/signed_log.py @
# 156ce74e3d1af71009b83f3212ad67238e23eaa2. Only the two imports below were
# swapped; every other line is verbatim.
#   * ``from identity.openclaw_identity import OpenClawIdentity``
#       -> local Ed25519 adapter (same .public_key/.network/.sign surface)
#   * ``from shared.logging import get_logger`` -> stdlib logging
# The original ``get_logger`` returned a structlog-style logger whose .debug
# accepts arbitrary keyword fields. Stdlib ``Logger.debug`` does not, so we
# wrap it in a thin adapter that swallows kwargs, keeping the .debug(...) call
# sites in this file byte-for-byte verbatim.
from .identity import Ed25519Identity as OpenClawIdentity


class _StructuredLoggerAdapter:
    """Thin stdlib-logging adapter accepting structured keyword fields."""
...
def get_logger(name: str) -> _StructuredLoggerAdapter:
    return _StructuredLoggerAdapter(logging.getLogger(name))


# --- end vendor import swaps -------------------------------------------------
...
class SignedAppendOnlyLog(_LegacyAppendOnlyBase):
    ...
    * ``signature`` — hex-encoded Ed25519 signature over the canonical JSON
    ...
    is intentional. The self-entry round-trip and chain semantics are covered
    by ``tests/test_signed_log_roundtrip.py``; the witness / foreign-entry /
    downgrade surface by ``tests/test_signed_log_witness.py``; and the
    independent verifier CLI by ``tests/test_verify_cli.py``.
